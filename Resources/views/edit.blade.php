        ...
        <div class="row">
            <div class="col-lg-12 col-md-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form action="{{ route('News.update', $News->id) }}" method="post" enctype="multipart/form-data" class="row g-3 needs-validation" novalidate>
                            ...
                            <div class="col-md-12">
                                ...
                                <select name="tags[]" class="form-control select2" multiple>
                                    @foreach($tags as $tag)
                                        <option value="{{ $tag->id }}" @if(in_array($tag->id, $News_tags)) selected @endif>{{ $tag->name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-12">
                                ...
                                <input type="text" name="title" class="form-control" id="title" required value="{{ $News->title }}">
                                <div class="invalid-feedback">لطفا عنوان را وارد کنید</div>
                            </div>
                            <div class="col-md-12">
                                <label for="slug" class="form-label">نامک</label>
                                <input type="text" name="slug" class="form-control" id="slug" required value="{{ $News->slug }}">
                                <div class="invalid-feedback">لطفا نامک را وارد کنید</div>
                            </div>
                            <div class="col-md-12">
                                <label for="short_text" class="form-label">توضیح کوتاه</label>
                                <input type="text" name="short_text" class="form-control" id="short_text" value="{{ $News->short_text }}" required>
                                <div class="invalid-feedback">لطفا توضیح کوتاه را وارد کنید</div>
                            </div>

                            <div class="col-md-12">
                                <label for="editor1" class="form-label">متن</label>
                                <textarea id="editor1" name="body" class="cke_rtl" required>{{ $News->body }}</textarea>
                                <div class="invalid-feedback">لطفا متن را وارد کنید</div>
                            </div>
                            <div class="col-md-5">
                                <label for="image" class="form-label">تصویر شاخص</label>
                                <input type="file" name="image" class="form-control" aria-label="تصویر شاخص" id="image" accept="image/*" @if(!$News->image) required @endif>
                                <div class="invalid-feedback">لطفا یک تصویر انتخاب کنید</div>
                            </div>
                            <div class="col-md-1">
                                @if($News->image)
                                    <label for="image" class="form-label">تصویر فعلی</label>
                                    <img src="{{ url($News->image) }}" style="max-width: 50%;">
                                @endif
                            </div>
                            <div class="col-md-6">
                                <label for="alt" class="form-label">متن جایگزین تصویر شاخص</label>
                                <input type="text" name="image_alt" class="form-control" value="{{ $News->image_alt }}">
                                <div class="invalid-feedback">لطفا متن جایگزین تصویر شاخص را وارد کنید</div>
                            </div>

                            <div class="col-md-11">
                                <label for="banner" class="form-label">بنر</label>
                                <input type="file" name="banner" class="form-control" aria-label="بنر" accept="image/*" @if(!$News->banner) required @endif>
                                <div class="invalid-feedback">لطفا یک تصویر انتخاب کنید</div>
                            </div>
                            <div class="col-md-1">
                                @if($News->banner)
                                    <label for="banner" class="form-label">تصویر فعلی</label>
                                    <img src="{{ url($News->banner) }}" style="max-width: 50%;">
                                @endif
                            </div>

{{--                            <div class="col-md-5">--}}
{{--                                <label for="photo" class="form-label">تصویر داخلی</label>--}}
{{--                                <input type="file" name="photo" class="form-control" aria-label="تصویر داخلی" accept="image/*" @if(!$News->photo) required @endif>--}}
{{--                                <div class="invalid-feedback">لطفا یک تصویر انتخاب کنید</div>--}}
{{--                            </div>--}}
{{--                            <div class="col-md-1">--}}
{{--                                @if($News->photo)--}}
{{--                                    <label for="photo" class="form-label">تصویر فعلی</label>--}}
{{--                                    <img src="{{ url($News->photo->path) }}" style="max-width: 50%;">--}}
{{--                                @endif--}}
{{--                            </div>--}}
{{--                            <div class="col-md-6">--}}
{{--                                <label for="alt" class="form-label">متن جایگزین تصویر داخلی</label>--}}
{{--                                <input type="text" name="photo_alt" class="form-control" @if($News->photo) value="{{ $News->photo->alt }}" @endif>--}}
